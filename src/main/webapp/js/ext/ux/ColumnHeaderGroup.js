        ...
        renderHeaders: function(){
            ...
            for(var row = 0, rlen = rows.length; row < rlen; row++){
                ...
                for(var i = 0, gcol = 0, len = r.length; i < len; i++){
                    ...
                    var colIndex = group.dataIndex ? cm.findColumnIndex(group.dataIndex) : gcol;
                    if(row == rlen-1){
                    	//最底下的一层分组（不计算表格的header这一组）
                    	if(group.colspan == 1){
	                    	cm.config[colIndex+group.colspan-1].marginTop = true;
	                    }
                    }
                    var id = this.getColumnId(colIndex), gs = Ext.ux.grid.ColumnHeaderGroup.prototype.getGroupStyle.call(this, group, gcol);
...
//                    	cls : 'ux-grid-hd-group-cell',
		...
		onColumnWidthUpdated: function(){
            ...
            //2015-1-23 xuqc不知道为什么，如加上一个延时，导致后面的列没有及时调整
