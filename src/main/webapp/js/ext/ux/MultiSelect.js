...
/**
 * 这个类在ItemSelector.js中使用，主要用于类似赋予角色的左右选择框，可以通过左右拖动来实现赋值。
...
//    height:100,
    ...
    /**
     * 添加记录
     * [{valueField:'123',displayField:'abc'}]
     * @param {} valueAry
     */
    addRecord : function(valueAry){
    	if(valueAry && valueAry.length > 0){
    		var store = this.view.store;
    		for(var i=0;i<valueAry.length;i++){
    			var record = new Ext.data.Record(valueAry[i]);
    			store.add(record);
    		}
    		this.view.refresh();
    	}
    	this.fireEvent('_add',this,valueAry);
    },
    /**
     * 更新记录,只能更新一条记录
     * {valueField:'123',displayField:'abc'}
     * @param {} valueAry
     */
    updateRecord : function(valueObj){
		var store = this.view.store;
		//读取当前选中的行
		var selectionsArray = this.view.getSelectedIndexes();
		var record = store.getAt(selectionsArray[0]);
		record.set(this.valueField,valueObj[this.valueField]);
		record.set(this.displayField,valueObj[this.displayField]);
		this.view.refresh();
		this.fireEvent('_update',this,valueObj);
    },
    /**
     * 删除记录，先根据提供的值进行检索，找到相应记录后删除，若没找到，则不删除
     * ['123','abc']
     * @param {} valueAry
     */
    removeRecord : function(valueAry){
    	if(valueAry && valueAry.length > 0){
    		var store = this.view.store;
    		for(var i=0;i<valueAry.length;i++){
    			var record = store.query(this.valueField, new RegExp('^' + valueAry[i] + '$', "i")).itemAt(0);
    			if(record){
    				store.remove(record);
    			}
    		}
    	}
    	this.fireEvent('_remove',this,valueAry);
    },
    /**
     * 清除所有记录
     */
    ...
    setRecord : function(valueAry){
    	...
    	this.fireEvent('_set',this,valueAry);
    },
    select : function(index){
    	...
    	this.fireEvent('_select',this,index);
    }
});
