...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
    init : function(grid) {
        ...
        if(grid.isAddBbar){
	        grid.store.on({load:this.refreshSummary, scope: this});         	
        }else{
		...
		this.cellTpl = new Ext.Template(
		    '<td class="x-grid3-col x-grid3-cell x-selectable x-grid3-td-{id} {css}" style="{style}">',
        ...
        for(var i=0;i<cfg.length;i++){
        	if(cfg[i].summaryType){
        		if(cfg[i].summaryType == 'max'){
        			this.title = '最大值';
        		}else if(cfg[i].summaryType == 'min'){
        			this.title = '最小值';
        		}else if(cfg[i].summaryType == 'average'){
        			this.title = '平均值';
        		}
        		break;
        	}
    ...
    calculate : function(rs, cm) {
    	if(this.grid.isAddBbar){
    		//2013-4-26，如果是没有分页的表格，那么直接在页面统计就好，否则通过store.summaryRow作为统计数据
    		return this.recordData = this.grid.store.summaryRow||{};
    	}
        ...
        for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
            ...
            if (cf.summaryType && cf.summaryType!='') {
		...
		this.recordData = data;//兼容旧版本，recordData这个变量在其他地方使用过了
    ...
    renderSummary : function(o, cs, cm) {
        cs = cs || this.view.getColumnData(true);
        var cfg = cm.config,
            buf = [],
            last = cs.length - 1;
        ...
        for (var i = 0, len = cs.length; i < len; i++) {
            ...
            //把合计放在第二列,这样可以缩小行号这一列的要求,报销系统的要求,
            //但是如果遇到第二列刚好需要统计的话就不行
            if(i==0){
            	buf[buf.length] = '<td class="x-grid3-col x-grid3-cell" style="'+p.style+'"><div class="x-grid3-cell-inner" unselectable="on">'+this.title+'</div></td>';
            }else if(cf.summaryType || cf.summaryRenderer){
            	...
            	//不需要统计的行
            	buf[buf.length] = '<td class="x-grid3-col x-grid3-cell" style="'+p.style+'">&nbsp;</td>';
            }
        }
    ...
    refreshSummary : function() {
        var g = this.grid, ds = g.store,
            cs = this.view.getColumnData(true),
            cm = this.cm,
            rs = ds.getRange(),
