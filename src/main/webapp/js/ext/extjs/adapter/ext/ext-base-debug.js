...
(function(){
    var idSeed = 0,
        ...
        isIE8 = isIE && ((check(/msie 8/) && docMode != 7)||(check(/msie 9/) && docMode == 8)||(check(/msie 10/) && docMode == 8)),
        isIE9 = isIE && ((check(/msie 9/) && docMode == 9)||(check(/msie 10/) && docMode == 9)||(check(/msie 10/) && docMode == 10)),//IE10暂时使用IE9的模式
    ...
    //目前对IE9的支持还不是很好。会出现treePanel无法选择的问题。如果是IE9直接使用Chrome模式
    //有些函数以Ext.isIE判断，有不同的实现，这里将isIE置为false导致IE9下的实现不对。去掉这段代码，经测试treePanel不再存在问题。
...
// s now contains the string: '&lt;div class="my-class">Some text&lt;/div>'
    ...
    format : function(format){
        ...
        //FIXME 2011-09-30,先预编译
        reg.compile(reg);
        return format.replace(reg, function(m, i){
            return args[i];
        });
    }
//    format : function(s){
...
//		reg.compile(reg);
//		var arr = s.match(reg); //返回两个元素的数组，第一个是匹配的字符，第二个是出现的位置
//		var ss='';
//		while(arr){
//			var index = parseInt(arr[1]);
//			ss+= s.substring(0,index);
//			ss+=args[index];
//			s = s.substring(index+arr[0].length);
//			arr = s.match(reg);
//		}
//		ss+=s;
//		return ss;
