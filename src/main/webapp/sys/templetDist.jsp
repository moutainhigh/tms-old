...
<html>
	...
	<body>
		...
		var roleTree = new uft.extend.tree.Tree({
			id : 'roleTree',
			treeRootNodeText:'角色树', //默认根节点名称
			rootVisible : true,
			border : false,
			dataUrl : 'getRoleTree.json', //默认数据来源
			isTreeFilter:true
		});
		...
		Ext.extend(TempletDistToolbar,Ext.Toolbar, {
			btn_dist_handler : function(){
				...
				node = roleTree.getSelectedNode();
				if(!node || !node.leaf){
					uft.Utils.showWarnMsg('请选择角色树的叶子节点！');
					return false;
				}
				templetGrid.enable();
			...
			btn_save_handler : function(){
				...
				var pk_role = roleTree.getSelectedNode().id;
				var params = [];
			    ...
			    uft.Utils.doAjax({
			    	scope : this,
			    	params : {pk_fun:pk_fun,pk_role:pk_role,templetAry: Ext.encode(params)},
		...
		Ext.onReady(function(){
			...
			funTree.on('click',function(node,event){
				if(node.leaf){
					var roleNode = roleTree.getSelectedNode();
					if(roleNode && roleNode.leaf){
						var options = {};
						options.params = {};
						options.params['pk_fun'] = node.id;
						options.params['pk_role'] = roleNode.id;
			...
			roleTree.on('click',function(node,event){
				if(node.leaf){
					var funNode = funTree.getSelectedNode();
					if(funNode && funNode.leaf){
						var options = {};
						options.params = {};
						options.params['pk_fun'] = funNode.id;
						options.params['pk_role'] = node.id;
						templetGrid.getStore().reload(options);
						statusMgr.setPageStatus(uft.jf.pageStatus.OP_NOTEDIT_CARD);
						statusMgr.updateStatus();
						return;
			...
			templetGrid.getStore().on('load',function(store,records,options){
			    ...
			    uft.Utils.doAjax({
			    	scope : this,
			    	params : {pk_fun:pk_fun,pk_role:pk_role},
