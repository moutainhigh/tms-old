	...
	<script type="text/javascript">
		...
		MyToolbar = Ext.extend(uft.jf.ToftToolbar, {
			...
			btn_add_handler : function(){
				...
				Ext.getCmp('fun_code').setValue(fun_code);
		...
		Ext.getCmp('_pk_corp').on('change',function(field,value,originalValue){
			var node = funTree.getSelectedNode();
			var ds = app.headerGrid.getStore();
			if(node){
				//先切换到列表页
				var sm = app.statusMgr;
				sm.setPageStatus(uft.jf.pageStatus.OP_NOTEDIT_LIST);
				sm.updateStatus();
				ds.baseParams['pk_corp'] = value;
				var options = {};
				options.params = {};
				options.params['pk_corp'] = value;
				ds.reload(options);
			}else{
				app.resetCardValues();
				app.clearBodyGrids();
