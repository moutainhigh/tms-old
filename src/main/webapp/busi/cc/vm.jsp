...
<link href="<c:url value="/busi/cc/etList.css"/>" rel="stylesheet" type="text/css" />
<link>
</head>
<body>
	<div id="container">
		...
		<div id="query_cmp">
			...
			<ul class="nav nav-tabs"  id="myTab" >
				<li class="active"><a href="#tab_0" data-toggle="tab" style="padding: 5px;"><font>全部(</font><span
						style="color: red;" id="tab_0_length">0</span><font>)</font>
				</a></li>
				<li class=""><a href="#tab_1" data-toggle="tab" style="padding: 5px;"><font>速度异常(</font><span
						style="color: red;" id="tab_1_length">0</span><font>)</font>
				</a></li>
				<li><a href="#tab_2" data-toggle="tab" style="padding: 5px;"><font>温度异常(</font><span
						style="color: red;" id="tab_2_length">0</span><font>)</font>
				</a></li>
				<li><a href="#tab_3" data-toggle="tab" style="padding: 5px;"><font>监控(</font><span
						style="color: red;" id="tab_3_length">0</span><font>)</font>
				</a></li>
			</ul>
			<div class="tab-content" >
				<div class="tab-pane active" id="tab_0" style="overflow-y: auto;">
					<table class="table table-bordered table-hover" id="fence_list_table">
						<tbody id="tab_0_list_tbody">
						</tbody>
					</table>
				
				</div>
				<div class="tab-pane" id="tab_1" style="overflow-y: auto;">
					<table class="table table-bordered table-hover" id="fence_list_table">
						<tbody id="tab_1_list_tbody">
						</tbody>
					</table>
				</div>
				<div class="tab-pane" id="tab_2" style="overflow-y: auto;">
					<table class="table table-bordered table-hover" id="fence_list_table">
						<tbody id="tab_2_list_tbody">
						</tbody>
					</table>
				</div>
...
<script type="text/javascript">
	...
	window.onload = function() {//用window的onload事件，窗体加载完毕的时候 
		...
		$.ajax({
			type : "GET",
			url : ctxPath + '/cc/vm/loadVehicleData.json',
			data : '',
	...
	function createQueryCmp(datalist) {
		...
		for (var i = 0; i < datalist.length; i++) {
			...
			if (!equipVO.speed_status || equipVO.speed_status != "在线") {
				//默认都是 超速，在线，离线
				speedWarnStr += createEquipInfoUnit(equipVO);
				speedWarnNum++;
			}
			if(!equipVO.temp_status || equipVO.temp_status != "正常")
			if (equipVO.speed_status == "温度异常") {
				tempWarnStr += createEquipInfoUnit(equipVO);
				tempWarnNum++;
			}
			watchStr += createWatchStr(equipVO);
			allStr += createEquipInfoUnit(equipVO);
			allNum++;
		}
		//表头信息，固定写死
		var head = "<tr class='' style='padding-bottom: 1px;'>"
						+"<th style='padding-bottom: 1px;padding-top: 1px; width: 30%; font-size: 12px;'>车辆</th>"
						+"<th style='padding-bottom: 1px;padding-top: 1px; width: 15%; font-size: 12px;'>状态</th>"
						+"<th style='padding-bottom: 1px;padding-top: 1px; width: 30%; font-size: 12px;'>速度(km/h)</th>"
						+"<th style='padding-bottom: 1px;padding-top: 1px; width: 25%; font-size: 12px;'>温度(°C)</th>"
					+"</tr>";
		document.getElementById("tab_0_list_tbody").innerHTML = head + allStr;
		document.getElementById("tab_0_length").innerHTML = allNum;
		document.getElementById("tab_1_list_tbody").innerHTML = head + speedWarnStr;
		document.getElementById("tab_1_length").innerHTML = speedWarnNum;
		document.getElementById("tab_2_list_tbody").innerHTML = head + tempWarnStr;
		document.getElementById("tab_2_length").innerHTML = tempWarnNum;
		//如果存在就不要再刷新了。
	...
	//查询锁定栏的数据 
	function createWatchStr(equipVO){
		var equipcode = equipVO.carno;
	...
	//设备列表的数据
	function createEquipInfoUnit(equipVO) {
		var carno = equipVO.carno;
		var temp = equipVO.temp1 +'<font color="red">&nbsp;|&nbsp;</font>' +equipVO.temp2;
		var speed = 0.00;
		if(equipVO.speed){
			speed = equipVO.speed;
		}
		var str = "<tr>";
		str += "<td onclick='equipClick("+ equipVO.longitude+ ","+ equipVO.latitude+ ")' style='cursor:pointer; font-size:12px; font-weight:bold; font-family:Verdana, Geneva, sans-serif'>"+carno+"</td>";
		if(equipVO.lot){
			str += "<td><img id='"+carno+"' style='cursor:pointer' onclick='showVehicleInfo(this.id)' src='<c:url value='/busi/cc/images/ydd.png'/>'></td>";
		}else{
			str += "<td><img src='<c:url value='/busi/cc/images/wdd.png'/>'></td>";
		}
		str += "<td>"+speed+"</td>";
		str += "<td>"+temp+"</td>";
		str += "</tr>";
	...
	//设备详情展示窗口
	function showVehicleInfo(carno) {
		var equipInfo="";
		$.ajax({
			type : "GET",
			url : ctxPath + '/te/et/loadEquipLotsOrdersInfo.json',
			data : {equipcode : carno},
			async: false,
			success : function(data) {
				equipInfo = data;
			}
	...
	$('#keyword').keyup(function() {
		...
		$.ajax({
			type : "GET",
			url : ctxPath + '/cc/vm/loadQueryVehicleData.json',
	...
	function createEquipInfoByLock(datalist){
		...
		for (var i = 0; i < datalist.length; i++) {
			...
			points.push(pt);
			var myIcon = new BMap.Icon(ctxPath + "/images/car_icon/green_2.gif", new BMap.Size(28, 28));
	...
	function createContent(equipVO){
		var temp1 = equipVO.temp1 + '';
		if(!temp1){
			temp1 = ' ';
		}
		var temp2 = equipVO.temp2 + '';
		if(!temp2){
			temp2 = ' ';
		}
		var temp3 = equipVO.temp3 + '';
		if(!temp3){
			temp3 = ' ';
		}
		var temp4 = equipVO.temp4 + '';
		if(!temp4){
			temp4 = ' ';
		}
		var memo = equipVO.memo;
		if(!memo){
			memo = '';
		}
		//标注信息
		var content = '<div style="font-size:10px;height:auto;">';
		content += "车牌号：" + equipVO.carno;
		content += "<br/>";
		...
		content += "温度4："+temp4+"℃  ";
		content += "<br/>";
		content += "油耗：10.5 百公里/升 ,  压力：2.1MPa ";
		content += "<br/>";
	...
	function getEquipsByLock(){
		...
		if(document.getElementById("watchFlag").checked){
			...
			if(watchs && watchs.length-1 > 0){
				...
				$.ajax({
					type : "GET",
					url : ctxPath + '/cc/vm/loadVehicleDataByLock.json',
