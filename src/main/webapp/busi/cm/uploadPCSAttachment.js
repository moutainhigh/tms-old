Ext.ns('uft.cm');
uft.cm.uploadPCSAttachment = function(config){
	Ext.apply(this,config);
	if(!this.record){
		uft.Utils.showErrorMsg('请先选择一条记录！');
		return false;
	}
	var values = Utils.doSyncRequest(ctxPath+'/attach/attach/getTempletMap.json',{funCode:'t4061',tabCode:'ts_attachment'},'POST');
	if(!values || !values.data){
		if(langague && langague == 'en_US'){
			uft.Utils.showErrorMsg('No template data,funCode：t4061,tabcode:ts_attachment!');
		}else{
			uft.Utils.showErrorMsg('没有模板数据，funCode：t4061,tabcode:ts_attachment！');
		}
		return false;
	}
	...
	var uploadUrl = ctxPath+'/attach/attach/uploadAttachment.do?pk_pay_check_sheet='+this.record.data['pk_pay_check_sheet']+'&funCode=t4061';
	...
	uft.cm.uploadPCSAttachment.superclass.constructor.call(this, {
		title : '附件管理',
		width : 800,
		height : 500,
		collapsible : false,
...
Ext.extend(uft.cm.uploadPCSAttachment,Ext.Window, {
});
