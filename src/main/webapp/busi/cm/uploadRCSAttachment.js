Ext.ns('uft.cm');
uft.cm.uploadRCSAttachment = function(config){
	Ext.apply(this,config);
	if(!this.record){
		uft.Utils.showErrorMsg('请先选择一条记录！');
		return false;
	}
	var values = Utils.doSyncRequest(ctxPath+'/attach/attach/getTempletMap.json',{funCode:'t4051',tabCode:'ts_attachment'},'POST');
	if(!values || !values.data){
		if(langague && langague == 'en_US'){
			uft.Utils.showErrorMsg('No template data,funCode：t4051,tabcode:ts_attachment!');
		}else{
			uft.Utils.showErrorMsg('没有模板数据，funCode：t4051,tabcode:ts_attachment！');
		}
		return false;
	}
	...
	var uploadUrl = ctxPath+'/attach/attach/uploadAttachment.do?pk_rece_check_sheet='+this.record.data['pk_rece_check_sheet']+'&funCode=t4051';
	...
	uft.cm.uploadRCSAttachment.superclass.constructor.call(this, {
		title : '附件管理',
		width : 800,
		height : 500,
		collapsible : false,
...
Ext.extend(uft.cm.uploadRCSAttachment,Ext.Window, {
});
