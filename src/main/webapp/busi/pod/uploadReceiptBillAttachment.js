 ...
 * @param {} config
 */
uft.pod.uploadReceiptBillAttachment = function(config){
	Ext.apply(this,config);
	if(!this.record){
		uft.Utils.showErrorMsg('请先选择一条记录！');
		return false;
	}
	var values = Utils.doSyncRequest(ctxPath+'/attach/attach/getTempletMap.json',{funCode:'t604',tabCode:'ts_attachment'},'POST');
	if(!values || !values.data){
		if(langague && langague == 'en_US'){
			uft.Utils.showErrorMsg('No template data,funCode：t604,tabcode:ts_attachment！');
		}else{
			uft.Utils.showErrorMsg('没有模板数据，funCode：t604,tabcode:ts_attachment！');
		}
		return false;
	}
	...
	var uploadUrl = ctxPath+'/attach/attach/uploadAttachment.do?pk_invoice='+this.record.data['pk_invoice']+'&funCode=t604';
	...
	uft.pod.uploadSignBillAttachment.superclass.constructor.call(this, {
		title : '签单附件',
		width : 800,
		height : 500,
		collapsible : false,
...
Ext.extend(uft.pod.uploadReceiptBillAttachment,Ext.Window, {
});
