 ...
 * @param {} config
 */
uft.pod.uploadSignBillAttachment = function(config){
	Ext.apply(this,config);
	if(!this.record){
		uft.Utils.showErrorMsg('请先选择一条记录！');
		return false;
	}
	var values = Utils.doSyncRequest(ctxPath+'/attach/attach/getTempletMap.json',{funCode:'t603',tabCode:'ts_attachment'},'POST');
	if(!values || !values.data){
		if(langague && langague == 'en_US'){
			uft.Utils.showErrorMsg('No template data,funCode：t603,tabcode:ts_attachment!');
		}else{
			uft.Utils.showErrorMsg('没有模板数据，funCode：t603,tabcode:ts_attachment！');
		}
		return false;
	}
	...
	var uploadUrl = ctxPath+'/attach/attach/uploadAttachment.do?pk_invoice='+this.record.data['pk_invoice']+'&funCode=t603';
...
Ext.extend(uft.pod.uploadSignBillAttachment,Ext.Window, {
});
