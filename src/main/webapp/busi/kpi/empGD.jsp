...
<html>
	<head>
		...
		<script type="text/javascript" src="<c:url value="/js/ui-select.js?v=${version}" />"></script>
		<link href="<c:url value="/js/ui-select.css?v=${version}"/>" rel="stylesheet" type="text/css"></link>			
	</head>
	<body>
	...
	window.onload=function(){//用window的onload事件，窗体加载完毕的时候
		$.ajax({
			type:"GET",
       		url:'<%=request.getContextPath()%>/report/getEmp/unitData.json',
       		data:'',
		 ...
		 $.ajax({
		        ...
		        url:"<%=request.getContextPath()%>/report/getEmp/getEmp.json",
		        success:function(result){
		        	var data = eval(result);
		        	for(var i=0;i<data.length;i++){
		        		 $("#emp").append("<option value='"+data[i].name+"'>"+data[i].name+"</option>");
		        		}
		       		 },
		  ...
		  $.ajax({
		        ...
		        dataType:"json",
		        contentType:"application/json;charset=utf-8",
		        url:"<%=request.getContextPath()%>/report/getEmp/getCorp.json",
		        success:function(result){
		        	var data = eval(result);
		        	for(var i=0;i<data.length;i++){
	...
	function reloadData(){
		...
		var corp=document.getElementById("corp").value;
		 $.ajax({
		        ...
		        dataType:"json",
		        contentType:"application/json;charset=utf-8",
		        data:{"year":year,"month":month,"emp":emp,"corp":corp},
		        url:'<%=request.getContextPath()%>/report/getEmp/reloadData.json',
		        success:function(data){
		       		var datalist = eval(data);
		       		loadGrapData(datalist);
		        },
