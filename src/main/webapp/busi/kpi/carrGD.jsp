...
<html>
	...
	function loadGrapData(datalist){
		...
		for(var i=0;i<datalist.length;i++){
			...
			var licode = "li"+code;
			var acode = "a"+code;
			str += "<div style='background: #ffc; width: 200px;height:300px; float:left;'>"+
				"<div id="+ code +" style='width: 200px;height:200px' ></div>"+
				"<div id="+ text +" style='width: 200px;height:100px; font-size:14px;font-family:'微软雅黑';'>"+
				"</div></div> ";
		}
	...
	window.onload=function(){//用window的onload事件，窗体加载完毕的时候
		$.ajax({
			type:"GET",
       		url:'<%=request.getContextPath()%>/report/gd/unitCarrData.json',
       		data:'',
		 ...
		 $.ajax({
		        ...
		        url:"<%=request.getContextPath()%>/report/gd/getCarrier.json",
		        success:function(result){
		        	var data = eval(result);
		        	for(var i=0;i<data.length;i++){
		        		 $("#carr").append("<option value='"+data[i].name+"'>"+data[i].name+"</option>");
		        		}
		       		 },
		  ...
		  $.ajax({
		        ...
		        dataType:"json",
		        contentType:"application/json;charset=utf-8",
		        url:"<%=request.getContextPath()%>/report/gd/getCorp.json",
		        success:function(result){
		        	var data = eval(result);
		        	for(var i=0;i<data.length;i++){
	...
	function reloadData(){
		...
		var carr=document.getElementById("carr").value;
		var corp=document.getElementById("corp").value;
		 $.ajax({
		        type:"get",
		        dataType:"json",
		        contentType:"application/json;charset=utf-8",
		        data:{"year":year,"month":month,"carr":carr,"corp":corp},
		        url:'<%=request.getContextPath()%>/report/gd/reloadCarrData.json',
		        success:function(data){
		       		var datalist = eval(data);
		       		loadGrapData(datalist);
		        },
