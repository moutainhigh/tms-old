Ext.ns('uft.fleet');
uft.fleet.uploadAttachment = function(config){
	Ext.apply(this,config);
	if(!this.record){
		uft.Utils.showErrorMsg('请先选择一条记录！');
		return false;
	}
	var values = Utils.doSyncRequest(ctxPath+'/attach/attach/getTempletMap.json',{funCode:'t4033',tabCode:'ts_attachment'},'POST');
	if(!values || !values.data){
		if(langague && langague == 'en_US'){
			uft.Utils.showErrorMsg('No template data,funCode：t4033,tabcode:ts_attachment!');
		}else{
			uft.Utils.showErrorMsg('没有模板数据，funCode：t4033,tabcode:ts_attachment！');
		}
		return false;
	}
	...
	var id = this.record.get(this.app.getHeaderPkField());
	params['isBody'] = 'false';
	params['id'] = id;
	...
	var uploadUrl = ctxPath+'/attach/attach/uploadAttachment.do?id='+id+'&funCode='+this.funCode;
	...
	uft.fleet.uploadAttachment.superclass.constructor.call(this, {
		title : '附件管理',
		width : 800,
		height : 500,
		collapsible : false,
...
Ext.extend(uft.fleet.uploadAttachment,Ext.Window, {
});
