...
<body >
	...
	<c:forEach items="${resultsList}" var="result">
		<tr  bgcolor="#009FCC" >
			<td colspan="2"><a href="<%=request.getContextPath()%>/rf/sc/nodeScan.json?lot=${result.lot}&addr_name=${result.addr_name}&num=${result.num}&pk_ent_line_b=${result.pk_ent_line_b}&page=asscan.jsp" onClick="return checkStatus('${result.arrival_flag}');" ><font size="10px">${result.addr_name} </font> </a></td> <td colspan="1"><font size="10px">${result.num}</font></td> <td colspan="1"><font size="10px">${result.arrival_flag}</font></td>
		</tr>
	</c:forEach>
	
	<tr align="center"  bgcolor="#009FCC" >
		<td colspan="4"> <a href="<%=request.getContextPath()%>/busi/rf/as.jsp"><font size="10px" >返回</font></a>  </td>
	</tr>
	</table>
</body>
</html>
