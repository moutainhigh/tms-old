Ext.ns('uft.te');
/**
 * yaojie 2015 11 23 上传签单附件JSP
 * 上传POD签收单
 * @param {} config
 */
uft.te.uploadTrackingAttachment = function(config){
	Ext.apply(this,config);
	if(!this.records){
		uft.Utils.showErrorMsg('请先选中记录！');
		return false;
	}
	var values = Utils.doSyncRequest(ctxPath+'/attach/attach/getTempletMap.json',{funCode:'t512',tabCode:'ts_attachment'},'POST');
	if(!values || !values.data){
		if(langague && langague == 'en_US'){
			uft.Utils.showErrorMsg('No template data,funCode：t512,tabcode:ts_attachment！');
		}else{
			uft.Utils.showErrorMsg('没有模板数据，funCode：t512,tabcode:ts_attachment！');
		}
		return false;
	}
	...
	for(var i=0;i<rs.length;i++){
		ids.push(rs[i].get('pk_entrust'));//委托单PK
	}
	params['pk_entrust'] = ids;
	...
	var uploadUrl = ctxPath+'/attach/attach/uploadAttachment.do?pk_entrust='+ids+'&funCode=t512';
	...
	uft.te.uploadTrackingAttachment.superclass.constructor.call(this, {
		title : '跟踪附件',
		width : 800,
		height : 500,
		collapsible : false,
...
Ext.extend(uft.te.uploadTrackingAttachment,Ext.Window, {
});
