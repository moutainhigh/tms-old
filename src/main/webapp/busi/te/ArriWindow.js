...
/**
 */
uft.te.ArriWindow = Ext.extend(Ext.Window, {
	...
	constructor : function(config) {
		...
		com_tms_jf_ts_ent_line_b_detail_recordType.push({"name":"pk_ent_pack_b","type":"string"});
		...
		com_tms_jf_ts_ent_line_b_detail_recordType.push({"name":"unit_weight","type":"string"});
		com_tms_jf_ts_ent_line_b_detail_recordType.push({"name":"unit_volume","type":"string"});
		...
		com_tms_jf_ts_ent_line_b_detail_columns.push({"header":"<span class='uft-grid-header-column-not-edit'>pk_ent_pack_b</span>","width":100,"dataIndex":"pk_ent_pack_b","xtype":"gridcolumn","hidden":true,"editable":false});
		com_tms_jf_ts_ent_line_b_detail_columns.push({"header":"<span class='uft-grid-header-column-not-edit'>pk_entrust</span>","width":100,"dataIndex":"pk_entrust","xtype":"gridcolumn","hidden":true,"editable":false});
		com_tms_jf_ts_ent_line_b_detail_columns.push({"header":"<span class='uft-grid-header-column-not-edit'>单位重</span>","width":100,"dataIndex":"unit_weight","xtype":"gridcolumn","hidden":true,"editable":false});
		com_tms_jf_ts_ent_line_b_detail_columns.push({"header":"<span class='uft-grid-header-column-not-edit'>单位体积</span>","width":100,"dataIndex":"unit_volume","xtype":"gridcolumn","hidden":true,"editable":false});
		...
		this.gridDetail.on({'afteredit':function(e){
			if(e.field == 'num'){
				var unit_weight = e.record.get('unit_weight');
				var unit_volume = e.record.get('unit_volume');
				var num = e.record.get('num');
				if(unit_weight && num){
					e.record.set('weight',num*unit_weight);
				}else{
					e.record.set('weight',0);
				}
				if(unit_volume && num){
					e.record.set('volume',num*unit_volume);
				}else{
		...
		uft.te.ArriWindow.superclass.constructor.call(this, {
			title : this.title||'到货',
			width : 850,
			height : 450,
			collapsible : false,
	...
	confirmArrival : function (){
		...
		uft.Utils.doAjax({
			scope : this,
			params : params,
			url : ctxPath + '/te/tracking/arrival.json',
	...
	unconfirmArrival : function (){
		...
		uft.Utils.doAjax({
			scope : this,
			params : {APP_POST_DATA : Ext.encode(datas)},
			url : ctxPath + '/te/tracking/unArrival.json',
			success : function(values){
				//更新行信息
				if(values && values.datas){
					var datas = values.datas;
	    	...
	    	success : function(values){
	    		if(values && values.records){
	    		    ...
	    		    var p = new Ext.data.Record({
	    		    	"pk_ent_line_b":pk_ent_line_b_value,
	    		    	"addr_code": addr_code_value,
	    		    	"addr_name": addr_name_value,
	    		    	"pk_entrust":datas[i].pk_entrust,
	    		    	"pk_ent_pack_b":datas[i].pk_ent_pack_b,
	    		    	"pk_ent_line_pack_b":datas[i].pk_ent_line_pack_b,
	    		    	"goods_code":datas[i].goods_code,
	    		    	"goods_name":datas[i].goods_name,
	    		    	"num":datas[i].num,
	    		     	"pack_num_count":datas[i].pack_num_count,
	    		     	"plan_num":datas[i].plan_num,
	    		     	"plan_pack_num_count":datas[i].plan_pack_num_count,
	    		     	"pack":datas[i].pack,
	    		     	"weight":datas[i].weight,
	    		     	"volume":datas[i].volume,
	    		     	"unit_weight":datas[i].unit_weight,
	    		     	"unit_volume":datas[i].unit_volume
	    	            });
	        			store.insert(i,p);
	        		}
	    		    grid.getStore().reload();
