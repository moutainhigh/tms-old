Ext.ns('uft.te');
uft.te.uploadExpAccdentAttachment = function(config){
	Ext.apply(this,config);
	if(!this.record){
		uft.Utils.showErrorMsg('请先选择一条记录！');
		return false;
	}
	var values = Utils.doSyncRequest(ctxPath+'/attach/attach/getTempletMap.json',{funCode:'t514',tabCode:'ts_attachment'},'POST');
	if(!values || !values.data){
		if(langague && langague == 'en_US'){
			uft.Utils.showErrorMsg('No template data,funCode：t514,tabcode:ts_attachment！');
		}else{
			uft.Utils.showErrorMsg('没有模板数据，funCode：t514,tabcode:ts_attachment！');
		}
		return false;
	}
	...
	params['isBody'] = 'false';
	params['pk_exp_accident'] = this.record.data['pk_exp_accident'];
	...
	var uploadUrl = ctxPath+'/attach/attach/uploadAttachment.do?pk_exp_accident='+this.record.data['pk_exp_accident']+'&funCode=t514';
	...
	uft.te.uploadExpAccdentAttachment.superclass.constructor.call(this, {
		title : '附件管理',
		width : 800,
		height : 500,
		collapsible : false,
...
Ext.extend(uft.te.uploadExpAccdentAttachment,Ext.Window, {
});
