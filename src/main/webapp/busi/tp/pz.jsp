...
<html>
	...
	<body>
	<nw:Bill templetVO="${templetVO}" isBuildHeaderGrid="false" bodyGridsPagination="false,false,false,false" 
	bodyGridsDataUrl="loadSegmentByPKs.json,loadLineInfo.json,loadEntTransbilityB.json,refreshPayDetail.json" 
	...
	<script type="text/javascript">
		...
		entLineGrid.getStore().reload(options);
		...
		function afterChangeReq_deli_date(field,value,originalValue){
			var value_date = Date.parseDate(value,this.DATETIME_FORMAT);
			...
			//将所有线路信息的的时间都改掉
			var grid = Ext.getCmp('ts_ent_line_b'),store = grid.getStore(),count = store.getCount();
			for(var i=0;i<count;i++){
				var record = store.getAt(i);
		...
		function afterChangeReq_arri_date(field,value,originalValue){
			...
			for(var i=0;i<count;i++){
				var record = store.getAt(i);
				uft.Utils.setColumnValue(record,"req_leav_date",value);
			}
		}
