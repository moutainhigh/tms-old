...
<html>
	...
	<body>
		...
		bodyGridsDragDropRowOrder="true,true,true,true,true"/>			
	</body>
	<script type="text/javascript">
		...
		MyToolbar = Ext.extend(uft.jf.BillToolbar, {
			constructor : function(config){
				...
				this.btn_revise.enabledBizStatus = [uft.jf.bizStatus.INV_CONFIRM,uft.jf.bizStatus.INV_DELIVERY,uft.jf.bizStatus.INV_PART_DELIVERY,uft.jf.bizStatus.INV_ARRIVAL,uft.jf.bizStatus.INV_PART_ARRIVAL,uft.jf.bizStatus.INV_SIGN];
			},
...
// 				orderno.setReadOnly(true);
		...
		${moduleName}.appUiConfig.toolbar.on('beforesave',function(toolbar,params){
			//判断起点和终点信息
			var line_grid = Ext.getCmp('ts_inv_line_b');
			var line_store = line_grid.getStore(), line_count = line_store.getCount();
			if(line_count > 1){
				var firstRecord = line_store.getAt(0);
				var lastRecord = line_store.getAt(line_count-1);
				var first_operate = firstRecord.get("operate_type");
				var last_operate = lastRecord.get("operate_type");
				if(first_operate != 1 || last_operate != 2){
					var r=confirm("线路中，起始点和终点的操作类型不一致，您确认要保存吗？");
					if(r==true){
			...
			var rece_grid = Ext.getCmp('ts_rece_detail_b');
			var rece_store = rece_grid.getStore();
			var pks = [];
			for(var i=0;i<rece_store.getCount();i++){
				var _record = rece_store.getAt(i);
		...
		var lineGrid = Ext.getCmp('ts_inv_line_b');
		lineGrid.addListener('mouseup',function(){    
		...
		function afterLineEdit(){
			var grid = Ext.getCmp('ts_inv_line_b');
			...
			//var headRecord = uft.Utils.getSelectedRecord(app.headerGrid);
			//headRecord.beginEdit();
			//修改提货信息
			uft.Utils.getField('pk_delivery').setValue(firstRecord.get('pk_address'));
			uft.Utils.getField('deli_city').setValue(firstRecord.get('pk_city'));
			uft.Utils.getField('deli_province').setValue(firstRecord.get('pk_province'));
			uft.Utils.getField('deli_area').setValue(firstRecord.get('pk_area'));
			uft.Utils.getField('deli_detail_addr').setValue(firstRecord.get('detail_addr'));
			uft.Utils.getField('deli_contact').setValue(firstRecord.get('contact'));
			uft.Utils.getField('deli_mobile').setValue(firstRecord.get('phone'));
			uft.Utils.getField('deli_phone').setValue(firstRecord.get('mobile'));
			uft.Utils.getField('deli_email').setValue(firstRecord.get('email'));
			//修改到货信息
			uft.Utils.getField('pk_arrival').setValue(lastRecord.get('pk_address'));
			uft.Utils.getField('arri_city').setValue(lastRecord.get('pk_city'));
			uft.Utils.getField('arri_province').setValue(lastRecord.get('pk_province'));
			uft.Utils.getField('arri_area').setValue(lastRecord.get('pk_area'));
			uft.Utils.getField('arri_detail_addr').setValue(lastRecord.get('detail_addr'));
			uft.Utils.getField('arri_contact').setValue(lastRecord.get('contact'));
			uft.Utils.getField('arri_mobile').setValue(lastRecord.get('phone'));
			uft.Utils.getField('arri_phone').setValue(lastRecord.get('mobile'));
			uft.Utils.getField('arri_email').setValue(lastRecord.get('email'));
			//设置行号
			for(var i=0;i<count;i++){
				var record = store.getAt(i);
				uft.Utils.setColumnValue(record,'serialno',(i+1)*10);
			}
			//headRecord.endEdit();
		}
		
		${moduleName}.appUiConfig.toolbar.on('afteredit',function(toolbar,params){
			afterLineEdit();
		},this);
		...
		//编辑件数后，计算重量和体积，更新表头的重量和体积
		function afterEditLineNumOfPack(record){
			var unit_weight = uft.Utils.getNumberColumnValue(record,'unit_weight');
			var unit_volume = uft.Utils.getNumberColumnValue(record,'unit_volume');
			var num = uft.Utils.getNumberColumnValue(record,'num'); //件数
			record.beginEdit();
		...
		function updateHeaderFeeWeightCount(){
			...
			if(pk_trans_type && pk_trans_type.length > 0){
					...
					var pk_trans_type = Ext.getCmp('pk_trans_type').getValue();
					if(!bala_customer || !pk_trans_type){
						return;
					}
					var deli_city = Ext.getCmp('deli_city').getValue();
					var arri_city = Ext.getCmp('arri_city').getValue();
					uft.Utils.doAjax({
				    	scope : this,
				    	params : {
		...
		function updateHeaderPackSummary(){
			...
			if(grid){
				var store = grid.getStore(), count = store.getCount();
				if(count > 0){
					//判断运输类型 这里做简单判断，谁的数量多，就统计谁的
					var pickupMap = {};
						pickupMap['count'] = 0;
						pickupMap['pack_num_count'] = 0;
						pickupMap['num_count'] = 0;
						pickupMap['weight_count'] = 0;
						pickupMap['volume_count'] = 0;
					var deliveryMap = {};
						deliveryMap['count'] = 0;
						deliveryMap['pack_num_count'] = 0;
						deliveryMap['num_count'] = 0;
						deliveryMap['weight_count'] = 0;
						deliveryMap['volume_count'] = 0;
					for(var i=0;i<count;i++){
						var record = store.getAt(i);
						var operate_type = record.get("operate_type");
						if(operate_type == 1){
							pickupMap['count'] += 1;
							pickupMap['pack_num_count'] += record.get("pack");
							pickupMap['num_count'] += record.get("num");
							pickupMap['weight_count'] += record.get("weight");
							pickupMap['volume_count'] += record.get("volume");
						}else if(operate_type == 2){
							deliveryMap['count'] += 1;
							deliveryMap['pack_num_count'] += record.get("pack");
							deliveryMap['num_count'] += record.get("num");
							deliveryMap['weight_count'] += record.get("weight");
							deliveryMap['volume_count'] += record.get("volume");
						}
					}
					if(pickupMap['count'] >= deliveryMap['count']){
						uft.Utils.getField('pack_num_count').setValue(pickupMap['pack_num_count']);
						uft.Utils.getField('num_count').setValue(pickupMap['num_count']);
						uft.Utils.getField('weight_count').setValue(pickupMap['weight_count']);
						uft.Utils.getField('volume_count').setValue(pickupMap['volume_count']);
					}else{
						uft.Utils.getField('pack_num_count').setValue(deliveryMap['pack_num_count']);
						uft.Utils.getField('num_count').setValue(deliveryMap['num_count']);
						uft.Utils.getField('weight_count').setValue(deliveryMap['weight_count']);
						uft.Utils.getField('volume_count').setValue(deliveryMap['volume_count']);
					}
					
					/* var resultMap = uft.Utils.getGridSumValueMap('ts_inv_line_b',['pack_num_count','num','weight','volume']);
		...
		function afterEditLineAddress(record){
			afterLineEdit();
		}
		
		function afterEditLineReqDate(record){
			var grid = Ext.getCmp('ts_inv_line_b');
			var store = grid.getStore(), count = store.getCount();
			if(count == 0){
				return;
			...
			//headRecord.beginEdit();
			//修改提货信息
			uft.Utils.getField('req_deli_date').setValue(firstRecord.get('req_date_from'));
			//修改到货信息
			uft.Utils.getField('req_arri_date').setValue(lastRecord.get('req_date_from'));
			//headRecord.endEdit();
		}
