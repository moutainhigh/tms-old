...
//    maxLengthText : 'The maximum length for this field is {0}',
    //FIXME 多增加一个变量，将被中文替代
    minLengthText : 'The minimum length for this field is {0},{1}',
    maxLengthText : 'The maximum length for this field is {0},{1}',    
    /**
    ...
    preFocus : function(){
        ...
        //FIXME 如果存在字符，那么将焦点放到最后，这里为了可编辑表格中，当焦点放在单元格上，此时随便输入一个字符，直接切换到编辑状态，此时焦点放置的位置。
        //2013-4-12为了让selectOnFocus起作用，不要将焦点放在最后
//        if(!isEmpty){
//        	this.selectText(el.dom.value.length);
//        }
    },
    ...
    getErrors: function(value) {
        ...
        //FIXME 2012/2/10 fangw
        if(!this.disabled&&!this.readOnly){
			//支持中文文字验证，一个汉字两个字节
			var cArr = String(value).match(/[^\x00-\xff]/ig);
			var valueBytes=value.length + (cArr == null ? 0 : cArr.length);
			if (valueBytes < this.minLength) {
				errors.push(String.format(this.minLengthText, this.minLength,Math.floor(this.minLength/2)));
			}
			if (valueBytes > this.maxLength) {
				errors.push(String.format(this.maxLengthText, this.maxLength,Math.floor(this.maxLength/2)));
			}
        }        
        //原校验方式
