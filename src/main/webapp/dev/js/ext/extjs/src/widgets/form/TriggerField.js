...
Ext.form.TriggerField = Ext.extend(Ext.form.TextField,  {
    ...
    //FIXME XUQC readOnly状态下不再使用鼠标手
    updateEditState: function(){
        if(this.rendered){
            if (this.readOnly) {
                this.el.dom.readOnly = true;
                this.trigger.addClass('x-trigger-img-noedit');
                //this.el.addClass('x-trigger-noedit');
                this.mun(this.el, 'click', this.onTriggerClick, this);
                //this.trigger.setDisplayed(false);
            } else {
            	this.trigger.removeClass('x-trigger-img-noedit');
                if (!this.editable) {
                    this.el.dom.readOnly = true;
                    //this.el.addClass('x-trigger-noedit');
            ...
            //FIXME 不能使用this.with,我们使用tableLayout,this.width表示的是一个单位宽度
//            this.onResize(this.width || this.wrap.getWidth());
            //this.onResize(this.wrap.getWidth());
        }
    },
