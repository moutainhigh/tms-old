...
viewConfig: {
    ...
    headerTpl: new Ext.Template(
        '<table border="0" cellspacing="0" cellpadding="0" style="{tstyle}">',
            '<thead>',
                '<tr class="x-grid3-hd-row x-selectable">{cells}</tr>',//FIXME 增加一个x-selectable样式，鼠标可以选中单元格的text
            '</thead>',
        '</table>'
    ),
     ...
     * @type Ext.Template
     */
    cellTpl: new Ext.Template(//FIXME 增加一个x-selectable样式，鼠标可以选中单元格的text
...
//        return Ext.num(this.scrollOffset, Ext.getScrollBarWidth());
    	//FIXME 2012-06-28，这个19是滚动条的宽度，这里设置固定值，一方面是效率考虑，另一方面解决当存在横竖滚动条时，将横向滚动条拉到最右边导致错位问题
    	return Ext.num(this.scrollOffset, 19);
    },
    ...
    renderHeaders : function() {
        ...
        for (i = 0; i < colCount; i++) {
            ...
            if(colModel.config[i].marginTop){
            	properties.istyle += 'margin-top: -26px;';
            }
            
            cells[i] = headerTpl.apply(properties);
        }
        
        //FIXME XUQC 不使用String.format
     ...
     */
    getColumnStyle : function(colIndex, isHeader, isSummary) {
        var colModel  = this.cm,
            colConfig = colModel.config,
            //这里读取的是column的css属性觉得有点奇怪，感觉应该是style属性才是
...
//            style += String.format("text-align: {0};", align);
     ...
     */
    getColumnData : function(isSummary) {
        var columns  = [],
            colModel = this.cm,
            colCount = colModel.getColumnCount(),
        ...
        for (i = 0; i < colCount; i++) {
            ...
            columns[i] = {
                ...
                //FIXME
                beforeRenderer : colModel.beforeRenderer,
                scope   : colModel.getRendererScope(i),
                id      : colModel.getColumnId(i),
                style   : this.getColumnStyle(i,false,isSummary)
            };
        }
