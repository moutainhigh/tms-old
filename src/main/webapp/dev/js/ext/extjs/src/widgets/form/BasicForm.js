    ...
    setValues : function(values){
        if(Ext.isArray(values)){ // array of objects
            for(var i = 0, len = values.length; i < len; i++){
                ...
                if(f){
//                    f.setValue(v.value);
                	//FIXME 这里不再调用setValue方法，考虑到参照的特殊格式,该方法定义在所有组件的基类Field.js
                    f.setObjectValue(v.value);
...
//                    field.setValue(values[id]);
					//FIXME 这里不再调用setValue方法，考虑到参照的特殊格式,该方法定义在所有组件的基类Field.js
                    field.setObjectValue(values[id]);
                    if(this.trackResetOnLoad){
    ...
    getFieldValues : function(dirtyOnly){
        ...
        this.items.each(function(f) {
            if (!f.disabled && (dirtyOnly !== true || f.isDirty())) {
                ...
                key = o[n];
                val = f.getValue();
                //FIXME xuqc 对Date对象进行处理
                if(val instanceof Date){
                	val = val.dateFormat(f.format||'Y-m-d');
				}
