...
var combo = new Ext.form.ComboBox({
     ...
     */
    minListWidth : 120,//FIXME 2012-11-30，最小宽度适当加大，避免有些字符显示不全
    /**
    ...
    findRecord : function(prop, value){
        ...
        if(this.store.getCount() > 0){
            this.store.each(function(r){
            	//FIXME XUQC
            	var rv = r.data[prop];
            	if(this.numberflag === true){//数字型的，转成数字类型再比较
            		if(rv && value){
            			if(parseFloat(rv) == parseFloat(value)){
            				record = r;
            				return false;
            			}
            		}
            	}else{
            		if(String(rv) == String(value)){//FIXME 这里避免""==0这种情况的发生
	                    record = r;
	                    return false;
	                }
            	}
