...
//    validationEvent : 'keyup',
     ...
     */
//    validateOnBlur : true,
    validationEvent : false, //FIXME 2012/1/4，效率上的考虑，输入的时候不验证
    validateOnBlur : false, //同上    
    /**
...
var form = new Ext.form.FormPanel({
    ...
    setReadOnly : function(readOnly){
        ...
        //FIXME XUQC 加入readOnly的样式
        if(this.readOnly){
        	this.addClass('x-item-readOnly');
        	if(this.label){
        		if(this.allowBlank){
        	...
        	this.removeClass('x-item-readOnly');
        	if(this.label){
        		if(this.allowBlank){
     ...
     */
     validateValue : function(value) {
         ...
         if (error == undefined) {
             ...
             //this.markInvalid(error);//FIXME 2012/1/4，不使用qtip提示
             return false;
         }
     },
    ...
    setValue : function(v){
        ...
        if(this.rendered){
            ...
            //this.validate(); //FIXME 先不验证，等blur的时候再验证
        }
        return this;
    },
    //FIXME 增加setObjectValue方法，默认调用setValue，之类可以继承，这是为了兼容参照的数据返回格式
    setObjectValue : function(v){
