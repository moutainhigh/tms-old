 ...
 */
public class UFBooleanEditor extends PropertyEditorSupport {

	private final boolean allowEmpty;

	private final int exactDateLength = -1;// 不检查长度
	 ...
	 */
	public UFBooleanEditor() {
		this.allowEmpty = false;
	}

	public UFBooleanEditor(boolean allowEmpty) {
	...
	public void setAsText(String text) throws IllegalArgumentException {
		if(this.allowEmpty && !StringUtils.hasText(text)) {
			...
			if(!StringUtils.hasText(text)) {
				...
				if(text.contains(",") && text.split(",").length > 0) {
					setValue(UFBoolean.valueOf(text.split(",")[1]));
				} else {
					setValue(UFBoolean.valueOf(text));
				}
			}
		}
	}
	...
	public String getAsText() {
		UFBoolean value = (UFBoolean) getValue();
