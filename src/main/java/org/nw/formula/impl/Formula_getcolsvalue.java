 ...
 * 
 * <pre>
 * 格式示例：vadmindeptname,pk_unit->getColsValue("bd_deptdoc","deptname","def3","pk_deptdoc",vadmindeptid)
 * </pre>
 * 
 * @author fangw
 */
public class Formula_getcolsvalue extends AbstractFormula {
	...
	public Object getResult(Map<String, Object> rowContext) {
		...
		if(argList.get(argList.size() - 1) instanceof IFormula) {
			...
			// 如果参数不是子公式，则把变量翻译为值
			if(argList.get(argList.size() - 1) != null) {
				if(argList.get(argList.size() - 1).toString().startsWith("\"")
						&& argList.get(argList.size() - 1).toString().endsWith("\"")) {
					String t = argList.get(argList.size() - 1).toString();
					t = t.substring(1);
					t = t.substring(0, t.length() - 1);
					argList.set(argList.size() - 1, t);
				} else {
		...
		// 查询的字段
		for(int i = 1; i < argList.size() - 2; i++) {
			if(i > 1) {
				sb.append(",");
			}
		...
		// 参数名
		String param = argList.get(argList.size() - 2).toString();
		if(param.startsWith("\"")) {
		...
		try {
			if(argList.get(argList.size() - 1) == null) {
				...
				logger.debug("公式参数:" + argList.get(argList.size() - 1));
				list = this.getDao().queryForListWithCache(sb.toString(), argList.get(argList.size() - 1));
			}
		} catch(Exception e) {
