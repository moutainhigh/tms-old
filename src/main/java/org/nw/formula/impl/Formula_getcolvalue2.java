 ...
 * 根据主键从数据库查询特定字段的值,其功能类似SQL语句: select fieldname from tablename where pkfield1 =
 * pkvalue1 and pkfield2 = pkvalue2. 从这条SQL语句可以看出各个参数的含义. 示例：
 * nbudgetmoney->getColValue2(ycbx_yszxkz,nbudgetmoney,pk_jobmngfil,PK_JOBMNGFIL
 * ,pk_dept,PK_DEPT)
 * 
 * @author fangw
 */
public class Formula_getcolvalue2 extends AbstractFormula {
	...
	public Object getResult(Map<String, Object> rowContext) {
		...
		if(argList.get(5) instanceof IFormula) {
			// 如果第6个参数是子公式，则先执行子公式
			IFormula formula = (IFormula) argList.get(5);
			argList.set(5, formula.getResult(rowContext));
		} else {
			// 如果第6个参数不是子公式，则把变量翻译为值
			argList.set(5, rowContext.get(argList.get(5)));
		}
		...
		// 执行公式
		// getColValue2(tablename,fieldname,pkfield1,pkvalue1,pkfield2,pkvalue2)根据主键从数据库查询特定字段的值,其功能类似SQL语句:
		// select fieldname from tablename where pkfield1 = pkvalue1 and
		// pkfield2 = pkvalue2.
		...
		sb.append(" and ");
		sb.append(argList.get(4).toString());
		if(argList.get(5) == null) {
			sb.append(" is null");
		} else {
			sb.append("=?");
		}
		...
		try {
			if(argList.get(3) == null && argList.get(5) == null) {
				list = this.getDao().queryForListWithCache(sb.toString());
			} else if(argList.get(3) == null && argList.get(5) != null) {
				list = this.getDao().queryForListWithCache(sb.toString(), argList.get(5));
			} else if(argList.get(3) != null && argList.get(5) == null) {
				list = this.getDao().queryForListWithCache(sb.toString(), argList.get(3));
			} else {
				list = this.getDao().queryForListWithCache(sb.toString(), argList.get(3), argList.get(5));
			}
		} catch(Exception e) {
