...
/*KPI员工报表总览*/
@Service
public class EmpGrapDisplayServiceImpl extends TMSAbsReportServiceImpl {
	...
	public List<HashMap<String,String>> getData(String year, String month, String emp_code, String corp_code){
		final List<HashMap<String,String>> retList = new ArrayList<HashMap<String,String>>();
		//存储过程名称
		final String KPI_INDEX_EMP_PROC = "kpi_index_emp_proc";
		final String YRAR = year;
		final String MONTH = month;
		final String EMP = emp_code;
		...
		try {
			NWDao.getInstance().getJdbcTemplate().execute(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection conn) throws SQLException {
					// 设置存储过程参数
					int count = 4;
					String storedProc = DaoHelper.getProcedureCallName(KPI_INDEX_EMP_PROC, count);
	...
	public UserVO[] getEmpByCorpCond(){
		UserVO[] userVOs = NWDao.getInstance().queryForSuperVOArrayByCondition(UserVO.class,CorpHelper.getCurrentCorpWithChildren());
		return userVOs;
	}
