 ...
 * @date 2016 11 14
 */
public class MultiPointTransportationExcelImporter extends BillExcelImporter {
	
	private ContractService contractService = null;
	
	public MultiPointTransportationExcelImporter(ParamVO paramVO, IBillService service,ImportConfigVO configVO) {
	...
	public void _import(File file) throws Exception {
		...
		for(String key : groupMap.keySet()){
			...
			invoiceVO.setInvoice_origin(DataDictConst.INVOICE_ORIGIN.CC.intValue());
			invoiceVO.setInvoice_goods_type(DataDictConst.INVOICE_GOOD_TYPE.CS.intValue() + "");
			...
			if(transBilityBVOArrs != null && transBilityBVOArrs.length > 0
					&& StringUtils.isNotBlank(transBilityBVOArrs[0].getPk_car_type())){
		...
		for (InvLineBVO invLineBVO : allInvLineBVOs) {
			String key = invLineBVO.getPk_invoice();
		...
		for(String key : groupMapInvLineBVOs.keySet()){
			int index = 0;
			List<InvLineBVO> invLineBVOs = groupMapInvLineBVOs.get(key);
			//区分业务是一提多送还是多提一送
			//对lineBVOs按照pickup和delivery进行分组(这里只支持一点提多点送，和多点提一点送的业务，对多点提多点送的业务暂不支持!)
			Map<Integer,Map<String,List<InvLineBVO>>> groupMap1 = new HashMap<Integer, Map<String,List<InvLineBVO>>>();
			for(InvLineBVO lineBVO : invLineBVOs){
				Integer operation = lineBVO.getOperate_type();
				Map<String,List<InvLineBVO>> sameOperationLineVOs = groupMap1.get(operation);
				if(sameOperationLineVOs == null){
					sameOperationLineVOs = new HashMap<String, List<InvLineBVO>>();
					groupMap1.put(operation, sameOperationLineVOs);
				}
				String pk_address = lineBVO.getPk_address();
				List<InvLineBVO> sameAddressLineVOs = sameOperationLineVOs.get(pk_address);
				if(sameAddressLineVOs == null){
					sameAddressLineVOs = new ArrayList<InvLineBVO>();
					sameOperationLineVOs.put(pk_address, sameAddressLineVOs);
				}
				sameAddressLineVOs.add(lineBVO);
			}
			if(groupMap1.get(OperateTypeConst.PICKUP) == null || groupMap1.get(OperateTypeConst.DELIVERY) == null){
				throw new BusiException("多点运输单[?],提到货信息不完整，不能确认!");
			}
			if(groupMap1.get(OperateTypeConst.PICKUP).size() > 1 && groupMap1.get(OperateTypeConst.DELIVERY).size() >1){
				throw new BusiException("多点运输单[?],出现多点提送业务，此业务尚不支持!");
			}
			//一提多送业务
			if(groupMap1.get(OperateTypeConst.PICKUP).size() == 1){
				for (InvoiceVO invoiceVO : allInvoiceVOs){
					if(invoiceVO.getPk_invoice().equals(invLineBVOs.get(0).getPk_invoice())){
						invoiceVO.setTrans_type(DataDictConst.TRANSPORT_TYPE.YD.intValue());
					}
				}
				for(InvLineBVO invLineBVO : invLineBVOs){
			...
			//多提一送业务
			if(groupMap1.get(OperateTypeConst.DELIVERY).size() == 1){
				for (InvoiceVO invoiceVO : allInvoiceVOs){
					if(invoiceVO.getPk_invoice().equals(invLineBVOs.get(0).getPk_invoice())){
						invoiceVO.setTrans_type(DataDictConst.TRANSPORT_TYPE.DY.intValue());
					}
				}
				for(InvLineBVO invLineBVO : invLineBVOs){
