...
/**
 * 导出供应商信息，调用第三方提供的web service，将数据导出给第三方系统
 * 
 * @author songf
 * @Date 2015年11月23日 下午14:55:28
 *
 */
public class ExportSuppliersJobService implements IJobService {
	static Logger logger = Logger.getLogger(ExportSuppliersJobService.class);

	@SuppressWarnings("rawtypes")
	public void exec(JobDefVO jobDefVO) {
		logger.info("-------------------开始执行导出供应商信息-------------------");
		if (jobDefVO == null) {
			return;
		}

		logger.info("判断是否有供应商信息需要同步");
		...
		// soap尾
		String strTail = "</comtec>]]></cais:message><cais:commandName>ImportCustomer</cais:commandName></cais:SendMessage></soapenv:Body></soapenv:Envelope>";

		// 查询供应商信息
		String sql = "SELECT  supp_code AS id, supp_code AS code, supp_name AS name ,'Supplier' AS SupplierCode ,memo AS comment FROM ts_supplier WITH(NOLOCK) where ts_supplier.dr = 0 and isnull(ts_supplier.def1,'N') <>'Y'  ";
		...
		try {
			...
			//Element root = document.addElement("cais:message");
			//Element nodeComtec = root.addElement("comtec");
			// 获取数据
			List<HashMap> mapSuppliersList = NWDao.getInstance().queryForList(sql, HashMap.class);
			logger.info("共查询到" + mapSuppliersList.size() + "条供应商信息需要进行同步操作。");
			if (mapSuppliersList != null && mapSuppliersList.size() > 0) {
				for (int i = 0; i < mapSuppliersList.size(); i++) {
					HashMap addressRowMap = mapSuppliersList.get(i);
					pk_supplier = String.valueOf(addressRowMap.get("id"));
					String strCode = String.valueOf(addressRowMap.get("code"));
					String strName = String.valueOf(addressRowMap.get("name"));
					//String SupplierCode = String.valueOf(addressRowMap.get("suppliercode"));
					//String comment = String.valueOf(addressRowMap.get("comment"));

					// 创建message的xml片段
					Document document = DocumentHelper.createDocument();
					//Element nodeContact = nodeComtec.addElement("contact");
					Element nodeContact = document.addElement("contact");
					nodeContact.addElement("id").addText(strCode);
					nodeContact.addElement("code").addText(strCode);
					nodeContact.addElement("name").addText(strName);
					//nodeContact.addElement("comment").addText(comment);
					
					//Element node_contact_kind = nodeContact.addElement("contact_kind");
					//node_contact_kind.addElement("code").addText(SupplierCode);
					...
					routeVO.setCommandname("ImportCustomer");
					...
					if ("TRUE".equalsIgnoreCase(returnMsg)){
						...
						String updatesql1="UPDATE ts_supplier SET def1='Y'  WHERE supp_code='"+pk_supplier+"'";
