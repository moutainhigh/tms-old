 ...
 * 
 */
public class ExportJdRDOPJobService implements IJobService {

	static Logger logger = Logger.getLogger(ExportJdRDOPJobService.class);
	...
	public void exec(JobDefVO jobDefVO) {
		logger.info("-------------------开始执行导出应收调整信息-------------------");
		if (jobDefVO == null) {
			return;
		}

		logger.info("判断是否有应收调整信息需要同步");
